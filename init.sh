...
# Habilitar en catalogdb para metabase
echo "Habilitando extensión dblink en catalogdb..."
until $DOCKER exec catalogdb pg_isready -U "$CATALOG_DB_USER" -d "$CATALOG_DB_NAME" > /dev/null 2>&1; do
  sleep 2
done
$DOCKER exec -i catalogdb psql -U "$CATALOG_DB_USER" -d "$CATALOG_DB_NAME" <<EOSQL
CREATE EXTENSION IF NOT EXISTS dblink;
EOSQL
echo "dblink habilitado en catalogdb."
